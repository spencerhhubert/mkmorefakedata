...
def visualize_algo(
    config: Config, algorithm: Algorithm, save_path: Optional[str] = None
) -> None:
    ...
    if generated_grid is not None:
        if len(generated_grid.shape) == 1:
            ...
            ax1.imshow(
                generated_grid.data.reshape(1, -1),
                cmap="viridis",
                vmin=0,
                vmax=MAX_VALUE,
                aspect="auto",
    ...
    if transformed_grid is not None:
        if len(transformed_grid.shape) == 1:
            # For 1D grids, show as horizontal bar
            ax2.imshow(
                transformed_grid.data.reshape(1, -1),
                cmap="viridis",
                vmin=0,
                vmax=MAX_VALUE,
                aspect="auto",
    ...
    if rows is None or cols is None:
        ...
        if num_algorithms == 0:
            ...
            if cols * cols < num_algorithms:
                cols += 1
            rows = (
                num_algorithms + cols - 1
            ) // cols  # Equivalent to ceil(num_algorithms / cols)
    ...
    for i, algorithm in enumerate(algorithms[: rows * cols]):
        ...
        if generated_grid is not None:
            if len(generated_grid.shape) == 1:
                ...
                gen_ax.imshow(
                    generated_grid.data, cmap="viridis", vmin=0, vmax=MAX_VALUE
                )
        ...
        if transformed_grid is not None:
            if len(transformed_grid.shape) == 1:
                ...
                trans_ax.imshow(
                    transformed_grid.data, cmap="viridis", vmin=0, vmax=MAX_VALUE
                )
